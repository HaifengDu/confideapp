...
import { createNonceStr, getClientIp } from "./util";
// 引入项目的配置信息
 ...
 * @param req 
 */
export function recieve(req:express.Request){
    const body = req.body;
    if(!body){
        ...
        return Promise.reject(new ErrorMsg(false,"请求体为空"));
    }
    return new Promise<{[index:string]:boolean|any}>((resolve,reject)=>{
        xml2js.parseString(req.body,(err,data)=>{
            if(err){
                ...
                reject(new ErrorMsg(false,err.message,err));
                return;
            }
            if(!data||!data.xml){
                logHelper.errorOrder({
                ...
                reject(new ErrorMsg(false,"请求体未包含xml"));
                return;
            }
            const xml = data.xml;
            if(xml.result_code&&xml.result_code[0]==='SUCCESS'&&xml.return_code&&xml.return_code[0]==='SUCCESS'){
                ...
                resolve({data:xml,...new ErrorMsg(true,"支付成功")});
            }else{
                logHelper.errorOrder({
                    ip:getClientIp(req),
                    body:body,
                ...
                reject(new ErrorMsg(false,"支付失败"));
            }
        });
    });
...
export function wxPay(openid,orderNo,total_fee){
 ...
 * 统一下单
 */
function unifiedOrder(openid:string,orderNo:number,total_fee:number){
    //mch_id商户号
    //detail 支付详情
    //out_trade_no//商户订单号
    ...
    formData  += "</xml>";
    return new Promise<{prepay_id:string,[index:string]:any}>((resolve,reject)=>{
        request({url:url,method:'POST',body: formData,timeout:30000},(err,res)=>{
            ...
            if(res.statusCode!==200){
                reject(new ErrorMsg(false,"返回状态不正确"));
                return;
            }
            ...
            xml2js.parseString(res.body,(err,data)=>{
                ...
                resolve({
                    prepay_id:prepay_id,...new ErrorMsg(true)
                })
            });
        });
    });

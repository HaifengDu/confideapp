...
module.exports = {
    genUserSig: function(opt, cb) {
        //生成userSig
        try {
            var userSig = genUserSig(opt.userid, opt.sdkappid, opt.accountType, privateKey);
        } catch (error) {
            cb(error, null);
            return;
        }
    ...
    genPrivateMapKey: function(opt, cb) {
        //生成privateMapKey
        try {
            var privateMapKey = genPrivateMapKey(opt.userid, opt.sdkappid, opt.accountType, opt.roomid, privateKey);
        } catch (error) {
            cb(error, null);
            return;
        }
